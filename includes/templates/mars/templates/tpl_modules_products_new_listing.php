...
<ul class="productListingList newListingList">
  ...
  if ($products_new_split->number_of_rows > 0) {
    ...
    while (!$products_new->EOF) {
      ...
      if (PRODUCT_NEW_LIST_NAME != '0') {
        $display_products_name = '<h3 class="itemTitle"><a href="' . zen_href_link(zen_get_info_page($products_new->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($products_new->fields['master_categories_id']) . '&products_id=' . $products_new->fields['products_id']) . '">' . $products_new->fields['products_name'] . '</a></h3>';
      } else {
...
//            $how_many++;
      ...
      if (PRODUCT_NEW_LIST_DESCRIPTION > '0') {
        ...
        $display_products_description = '<div class="listingDescription">'.stripslashes(zen_trunc_string($disp_text, PRODUCT_NEW_LIST_DESCRIPTION, '<a href="' . zen_href_link(zen_get_info_page($products_new->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($products_new->fields['master_categories_id']) . '&products_id=' . $products_new->fields['products_id']) . '"> ' . MORE_INFO_TEXT . '</a>')).'</div>';
      } else {
        $display_products_description = '';
      }
