 ...
 */
?>
<ul class="productListingList featuredListingList">
  ...
  if ($featured_products_split->number_of_rows > 0) {
    ...
    while (!$featured_products->EOF) {
      ...
      if (PRODUCT_FEATURED_LIST_NAME != '0') {
        $display_products_name = '<h3 class="itemTitle"><a href="' . zen_href_link(zen_get_info_page($featured_products->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($featured_products->fields['master_categories_id']) . '&products_id=' . $featured_products->fields['products_id']) . '">' . $featured_products->fields['products_name'] . '</a></h3>';
      } else {
...
//            $how_many++;
      ...
      if (PRODUCT_FEATURED_LIST_DESCRIPTION > '0') {
        ...
        $display_products_description = '<div class="listingDescription">'.stripslashes(zen_trunc_string($disp_text, PRODUCT_FEATURED_LIST_DESCRIPTION, '<a href="' . zen_href_link(zen_get_info_page($featured_products->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($featured_products->fields['master_categories_id']) . '&products_id=' . $featured_products->fields['products_id']) . '"> ' . MORE_INFO_TEXT . '</a>')).'</div>';
      } else {
        $display_products_description = '';
      }
