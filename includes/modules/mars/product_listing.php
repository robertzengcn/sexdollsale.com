...
if ($listing_split->number_of_rows > 0) {
  ...
  if ($product_listing_layout_style == 'columns') {     
    switch (PRODUCT_LISTING_COLUMNS_PER_ROW) {
  ...
  // Setting the Product Listing Details Class
  $product_listing_details_display_class = ($product_listing_options->fields['product_listing_details_display'] == 2) ? " centerBoxContentsEffect" : "";  
...
// qty box with add to cart button
        ...
        if ($listing->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) {
          ...
          if (isset($_GET['manufacturers_id'])) {
            $lc_text = '<a class="listingProductLink" href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'cPath=' . (($_GET['manufacturers_id'] > 0 and $_GET['filter_id']) > 0 ?  zen_get_generated_category_path_rev($_GET['filter_id']) : ($_GET['cPath'] > 0 ? zen_get_generated_category_path_rev($_GET['cPath']) : zen_get_generated_category_path_rev($listing->fields['master_categories_id']))) . '&products_id=' . $listing->fields['products_id']) . '">' . zen_image(DIR_WS_IMAGES . $listing->fields['products_image'], $listing->fields['products_name'], IMAGE_PRODUCT_LISTING_WIDTH, IMAGE_PRODUCT_LISTING_HEIGHT, 'class="listingProductImage"') . '</a>';
          } else {
            $lc_text = '<a class="listingProductLink" href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'cPath=' . (($_GET['manufacturers_id'] > 0 and $_GET['filter_id']) > 0 ?  zen_get_generated_category_path_rev($_GET['filter_id']) : ($_GET['cPath'] > 0 ? zen_get_generated_category_path_rev($_GET['cPath']) : zen_get_generated_category_path_rev($listing->fields['master_categories_id']))) . '&products_id=' . $listing->fields['products_id']) . '">' . zen_image(DIR_WS_IMAGES . $listing->fields['products_image'], $listing->fields['products_name'], IMAGE_PRODUCT_LISTING_WIDTH, IMAGE_PRODUCT_LISTING_HEIGHT, 'class="listingProductImage"') . '</a>';
          }
        }
    ...
    if ($product_listing_layout_style == 'columns') {
      $new_product_contents = array_slice($product_contents, 0, 1, true) +
                              array("key_1" => '<div class="listingProductContent">') +
                              array_slice($product_contents, 1, count($product_contents)-1, true) + 
                              array("key_2" => '</div>');

      array_unshift($new_product_contents, '<div class="centerBoxContentsInner">');                        
      array_push ($new_product_contents, '</div>');                         
      
      $lc_text = implode('', $new_product_contents);        
      $list_box_contents[$rows][$column] = array('params' => 'class="centerBoxContentsProducts'.$product_listing_details_display_class.' centeredContent col '.$col_width.'"',
                                                 'text'  => $lc_text);
      $column ++;
  ...
  $list_box_contents[0][] = array('params' => 'class="productListing-data"',
                                              'text' => '<p class="no_products">'.TEXT_NO_PRODUCTS.'</p>');
