...
<div class="mb_inline_msg msg">
    <div class="mb_inline_msg img">
        <img src="<?php echo MH_CATALOG_SERVER . MH_DIR_WS_CATALOG;?>mailhive/common/images/mb_icon.png" width="32"
             height="32">
    </div>
                ...
                <div class="mb_inline_msg_inner item">
                    <div class="mb_inline_msg_inner item_content <?php echo ($module_info_array['price']) ? 'premium'
                        ...
                        <div class="mb_inline_msg_inner item_text <?php echo ($module_info_array['price']) ? 'premium'
                            : '' ?>">
                            <a href="<?php echo mh_language_documentation_url('http://www.mailbeez.com/documentation/'); ?><?php echo $tab . '/' . $key . '/' . MH_LINKID_1 ?>"
                               target="_blank">
...
$request_profiler->restart('---main_mailbeez_before_read_module_dir---');
...
for ($i = 0, $n = sizeof($directory_array); $i < $n; $i++) {
    ...
    $request_profiler->restart('main_mailbeez_before_load: ' . $class);

    mh_load_modules_language_files($module_directory_current, $class, MH_FILE_EXTENSION);
...
$request_profiler->restart('---main_mailbeez_before_module_list---');


while (list($sort_order_key, $file) = each($all_modules)) {
    $class = substr($file, 0, strrpos($file, '.'));
    $request_profiler->restart('main_mailbeez_before_display: ' . $class);

    if (is_object($GLOBALS[$class])) {
        $module = $GLOBALS[$class];
        $module->versioncheck_info = (($_SESSION['mailbeez_upd'][$module->code] > 0)
            ? '<span class="upd_cnt list"><a style="color: #fff" href="' . mh_language_documentation_url($module->documentation_root) . $module->documentation_key . '?update=true' . MH_LINKID_2 . '" target="_blank">' . MH_VERSIONCHECK_INFO_NEWVERSION . ' ' . $_SESSION['mailbeez_upd'][$module->code] . '</a></span>'
            : null); // experimental
    ...
    <td class="dataTableContent">
        ...
        <?php
        if ($module->check() < 1) {
            echo mh_image(MH_ADMIN_DIR_WS_IMAGES . 'install_bee.png', '', '', '', 'align="absmiddle" ');
        }
        ?>
        <?php
        ...
        if ($module->has_submodules && $module->show_submodules && $module->check() > 0) {
            echo mh_image(MH_ADMIN_DIR_WS_IMAGES . 'arrow_open.png', '', '', '', 'align="absmiddle" style="margin-left: 4px;"');
        } elseif ($module->has_submodules && $module->check() > 0) {
            echo mh_image(MH_ADMIN_DIR_WS_IMAGES . 'arrow_closed.png', '', '', '', 'align="absmiddle" style="margin-left: 4px;"');
        } elseif (isset($module->cert_check) && !$module->cert_check) {
            //echo '<span style="display: block; float: left; width: 17px">&nbsp;</span>';
            echo mh_image(MH_ADMIN_DIR_WS_IMAGES . 'lock.png', '', '13', '13', 'align="absmiddle" style="margin-left: 0px;"');
        ...
        <?php
        if ($module->icon != '') {
            echo mh_image(MH_ADMIN_DIR_WS_PRE . $module_directory_current_ws . $module->module . '/' . $module->icon, '', '13', '13', 'align="absmiddle"');
        }
        ?>
...
<?php if ($tab == 'mailbeez' && MH_SHOWMORELINK) { ?>
<tr class="dataTableRow">
    <td class="dataTableContent"><a href="http://www.mailbeez.com/download/<?php echo MH_LINKID_1;?>"
                                    target="_blank"><?php echo mh_image(MH_ADMIN_DIR_WS_IMAGES . 'more_beez.png', '', '', '', 'align="absmiddle"'); ?><?php echo MH_DOWNLOAD_LINK_LIST; ?></a>
    </td>
...
$module_icon = '';
if ($mInfo->icon != '') {
    $module_icon = mh_image(MH_ADMIN_DIR_WS_PRE . $module_directory_current_ws . $mInfo->module . '/' . $mInfo->icon, '', '', '', 'align="right"') . ' ';
}
...
switch ($action) {
    case 'edit':
        ...
        while (list($key, $value) = each($mInfo->keys)) {
            ...
            // hide layout control when visual editor installed
            if (stristr($key, '_CERT')) {
                continue;
            }
            if ((defined('MH_RESPONSIVE') && MH_RESPONSIVE == 'True') && stristr($key, '_LAYOUT') && defined('MAILBEEZ_CONFIG_EDITOR_STATUS') && MAILBEEZ_CONFIG_EDITOR_STATUS == 'True') {
                continue;
            }
    ...
    default:
        ...
        if ($mInfo->status == '1') {
            ...
            $certification_check_status = false;
            reset($mInfo->keys);
            while (list($key, $value) = each($mInfo->keys)) {
                if ($key == $module_key || $key == $config_module_key || $key == $filter_module_key ||
                    $key == $module_version_key || $key == $config_module_version_key || $key == $filter_module_version_key
                ...
                // hide layout control when visual editor installed
                if (stristr($key, '_LAYOUT') && defined('MAILBEEZ_CONFIG_EDITOR_STATUS') && MAILBEEZ_CONFIG_EDITOR_STATUS == 'True') {
                    continue;
                }


                if (stristr($key, '_CERT') && $mInfo->required_mb_version > 2.5) {
                    $cert_module = $mInfo->code;
                    $cert_key = $key;
                    $certification_check_status = $mInfo->cert_check;
                    if (file_exists(MH_DIR_FS_CATALOG . 'mailhive/configbeez/config_queen/admin_action_plugins/certificate_status.php')) {
                        require_once(MH_DIR_FS_CATALOG . 'mailhive/configbeez/config_queen/admin_action_plugins/certificate_status.php');
                    }
                    if ($certification_check_status !== true) {
                        if (file_exists(MH_DIR_FS_CATALOG . 'mailhive/configbeez/config_queen/admin_action_plugins/certificate.php')) {
                            require_once(MH_DIR_FS_CATALOG . 'mailhive/configbeez/config_queen/admin_action_plugins/certificate.php');
                        }
                        $keys .= '<div class="mb_action_box"></div>' . $certificate_plugin;
                        $certification_status = $certificate_status_plugin;
                    } else {
                        $keys .= '<div class="mb_action_box"></div>' . $certificate_status_plugin;

                    }

                } elseif (stristr($key, '_CERT')) {
                    $cert_module = $mInfo->code;
                    $cert_key = $key;
                    if (file_exists(MH_DIR_FS_CATALOG . 'mailhive/configbeez/config_queen/admin_action_plugins/certificate.php')) {
                        require_once(MH_DIR_FS_CATALOG . 'mailhive/configbeez/config_queen/admin_action_plugins/certificate.php');
            ...
            if ($mInfo->code == 'config' && !(MAILBEEZ_INSTALLED == 'config_queen.php' || MAILBEEZ_INSTALLED == '')) {
                $remove_button = '';
            }
            ...
            if (isset($mInfo->documentation_key)) {
                $contents[] = array('text' => '<br />' . mh_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '&nbsp;<b>' . TEXT_DOCUMENTATION . '</b> ' . $sversion . ' (<a href="' . mh_language_documentation_url($mInfo->documentation_root . $mInfo->documentation_key . MH_LINKID_1) . '" target="_blank">' . TEXT_VIEW_ONLINE . '</a>)');
            }
            /*
            ...
            */
            ...
            if ($certification_status) {
                $contents[] = array('text' => $certification_status);
            }
            ...
            if ($mInfo->code != 'config' && $mInfo->common_admin_action_plugins != '') {
                ...
                foreach ($common_admin_action_plugins_includes as $common_admin_action_plugins_includes) {
                    require_once(MH_DIR_FS_CATALOG . 'mailhive/common/admin_action_plugins/' . $common_admin_action_plugins_includes);
                }
            }
            ...
            if (defined('MAILBEEZ_CONFIG_TMPLMNGR_STATUS') || defined('MAILBEEZ_CONFIG_TMPLMNGR_LNG_STATUS')) {
                @mhpi('module_edit_content', $mInfo);
            } else {
                if ($mInfo->is_editable == true) {
                    require_once(MH_DIR_CONFIG . 'config_edit_promo/admin_action_plugins/edit.php');
                }
            }
...
if ((sizeof($all_modules) > 0) && defined('MAILBEEZ_MAILHIVE_STATUS') && (mh_not_null($heading)) && (mh_not_null($contents))) {
    echo '            <td width="25%" valign="top" id="cfg_column">' . "\n";
...
<div class="smallText" style="float: right; border: 0px solid #c0c0c0; padding: 5px;">
    ...
    <div style="float: left; padding-top: 2px;">
        <img src="<?php echo MH_CATALOG_SERVER . MH_DIR_WS_CATALOG ?>mailhive/common/images/been_tiny_love.png"
             width="49"
