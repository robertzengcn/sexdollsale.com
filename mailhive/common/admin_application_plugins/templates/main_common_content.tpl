                        ...
                        <tr>
                            ...
                            {if (defined('MAILBEEZ_VERSION'))}
                                <div class="mode_info {$MAILBEEZ_MAILHIVE_MODE}">{*$MAILBEEZ_VERSION_CHECK_BUTTON*}
                                    <a href="{$MAILBEEZ_MAILHIVE_MODE_SWITCH_URL}"><img
                                            src="{$MH_CATALOG_URL}mailhive/common/images/button_status_{$MAILBEEZ_MAILHIVE_MODE}.png"
                                            width="24" style="float: left" border="0"></a>

                                    <div style="margin-top: 5px;">{$MAILBEEZ_MAILHIVE_MODE_TEXT|lower}</div>
                                    {if $MAILBEEZ_MAILHIVE_MODE == 'simulate'}
                                        {$MAILBEEZ_MAILHIVE_MODE_SIM_RESTART}
                                    {/if}
                                </div>
                            {/if}
                            </td>
