...
//echo $this->config['uploadURL'];
//        $a = 'http://localhost/a';

        // Array ( [0] => http://localhost/a [1] => http [2] => localhost [3] => [4] => [5] => a )
...
//        exit();
        ...
        // MailBeez
        // issue URL check
        // only http://domain/path/ was recognized
...
//            list($unused, $protocol, $domain, $unused, $port, $path) = $patt;
            list($unused, $protocol, $domain, $unused, $port, $unused, $path) = $patt;
            $path = path::normalize($path);
//            $this->config['uploadURL'] = "$protocol://$domain" . (strlen($port) ? ":$port" : "") . "/$path";
            $this->config['uploadURL'] = "$protocol://$domain" . (strlen($port) ? ":$port" : "") . (strlen($path) ? "/$path" : "");
