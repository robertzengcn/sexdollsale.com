...
class Emogrifier
{
    ...
    public function emogrify()
    {
        ...
        // mailbeez, added , 'UTF-8', true
        // mailbeez, added custom encoding
        if (MAILBEEZ_CONFIG_EMAIL_ENGINE_ENCODING_SET != '') {
            $encoding = mb_detect_encoding($body, MAILBEEZ_CONFIG_EMAIL_ENGINE_ENCODING_SET, true);
        } else {
            $encoding = mb_detect_encoding($body, 'UTF-8', true);
        }
        ...
        foreach ($nodes as $node) {
            ...
            // mailbeez: avoid fatal error
            // Fatal error: Call to undefined method DOMElement::getNodePath()
            // increase html compatibility
    ...
    private function parseNth($match)
    {
        ...
        if (in_array(strtolower($match[2]), array('even', 'odd'))) {
            ...
            elseif ($multiplier == 0) return array(self::INDEX => $index); else $multiplier = intval($multiplier);
