...
if (isset($_GET['mi'])) {
    ...
    require_once($store_basedir . 'mailhive/common/classes/clicktracker.php');
    mb_clicktracker::record_open($_GET['mi'], $store_basedir);
    exit();
}


// WP Online Store, load wp environment
...
if (isset($_GET['apikey'])) {
    //if (stristr($_SERVER['PATH_INFO'], 'apikey')) {
    // api call
    require_once(MH_DIR_FS_CATALOG . 'mailhive/api/api.php');
    $api = new SHOP_API;
    $api->processApi();
    mh_exit();
}
...
if (!defined('MAILBEEZ_VERSION')) {
    include(MH_DIR_FS_CATALOG . 'mailhive/install/install.php');
    exit();
}

if (defined('MAILBEEZ_MAILHIVE_PROCESS_CONTROL_MEMORY_LIMIT')) {
    ini_set('memory_limit', MAILBEEZ_MAILHIVE_PROCESS_CONTROL_MEMORY_LIMIT);
}

if (!ini_get('safe_mode')) {
    if (defined('MAILBEEZ_MAILHIVE_PROCESS_CONTROL_TIME_LIMIT')) {
        set_time_limit(MAILBEEZ_MAILHIVE_PROCESS_CONTROL_TIME_LIMIT);
    }
}

if (MAILBEEZ_MAILHIVE_PROCESS_CONTROL_INGORE_USER_ABORT == 'True') {
...
$mailHive = new mailHive($_GET['module'], isset($_GET['mi']));
...
if (isset($_GET['cron_simple'])) {
    $inc_cron_path = MH_DIR_FS_CATALOG . 'mailhive/configbeez/config_cron_simple/includes/cron_simple_run.php';
...
if (isset($_GET['cron_advanced'])) {
    $inc_cron_path = MH_DIR_FS_CATALOG . 'mailhive/configbeez/config_cron_advanced/includes/cron_advanced_run.php';
    if (file_exists($inc_cron_path)) {
        require_once($inc_cron_path);
    }
    exit();
...
if ($output_plain == false) {
    ...
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=<?php echo ($mpaction == 'view') ? MAILBEEZ_CONFIG_EMAIL_ENGINE_ENCODING_SET : CHARSET; ?>">
...
<?php
    ...
    if ($mpaction == 'test') {
        ...
        foreach ($cfg_gender_array as $cfg_gnd_item => $cfg_gnd_item_text) {
            ...
            <div class="mh_choose">
                <div class="mh_choose_item"><a
                        ...
                        <?php  echo mh_image(MH_CATALOG_SERVER . DIR_WS_CATALOG . 'mailhive/filterbeez/filter_add_gender/' . $cfg_gnd_item . '_b.png', $cfg_gnd_item, '24', '24', 'align="absmiddle" width="24" height="24"') . ' ';
        ...
        <blockquote><br><br>
            <button type="button"
                    onclick="window.location.href='<?php echo MAILBEEZ_MAILHIVE_URL . 'run&module=' . $_GET['module']; ?>'">
<!--                    onclick="window.location.href='--><?php //echo mh_href_link_mailhive($mailhive_token . '=run&module=' . $_GET['module'], MH_REQUEST_TYPE) ?><!--'">-->
