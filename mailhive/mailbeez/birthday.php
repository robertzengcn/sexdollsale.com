...
// class methods
    function getAudience()
    {
        ...
        $query_raw = "select c.customers_firstname, c.customers_lastname, c.customers_id, c.customers_email_address, c.customers_dob, date_format(c.customers_dob, '%d.%m.') as date_of_birth
            ...
            where
                concat(year(now()), '', date_format(c.customers_dob,'%m%d')) < date_format( DATE_ADD(now(), INTERVAL " . MAILBEEZ_BIRTHDAY_BEFORE_DAYS . " DAY),'%Y%m%d') and
                concat(year(now()), '', date_format(c.customers_dob,'%m%d')) > date_format(SUBDATE(now(), INTERVAL " . MAILBEEZ_BIRTHDAY_PASSED_DAYS_SKIP . " DAY), '%Y%m%d')";
